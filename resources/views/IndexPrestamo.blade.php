    ...
    <div class="col-12 mt-4">
        <table class="table table-bordered text-white">
            ...
            @foreach ($prestamos as $prestamo)
                <tr>
                    <td class="fw-bold">{{ $prestamo->objeto->nombre ?? 'Objeto no encontrado' }} ({{ $prestamo->objeto->tipo ?? 'Tipo no encontrado' }})</td>
                    <td>{{ $prestamo->persona->nombre ?? 'Persona no encontrada' }} <br> {{ $prestamo->persona->telefono ?? 'Teléfono no encontrado' }} <br> {{ $prestamo->persona->email ?? 'Email no encontrado' }}</td>
